...
| Wan2.2 | [Wan-Video/Wan2.2](https://github.com/Wan-Video/Wan2.2) | Apache-2.0 | Released Jul 2025; dominant in mid-2026 | World's first open MoE video model. Runs on consumer GPUs. Ecosystem of fine-tuned LoRAs and ComfyUI workflows exploding on HuggingFace. |
| LTX-2.3 | [Lightricks/LTX-Video](https://github.com/Lightricks/LTX-Video) | Apache-2.0 | Released Mar 2026 | 22B DiT with native audio sync — first open model doing audio+video together. Production-ready 4K output is a qualitative leap above 2025 models. |
...
3. **MoE architecture arrives in video diffusion** — Wan2.2's MoE design achieves frontier quality at 5B active params, making studio-grade video generation feasible on consumer hardware for the first time.

4. **Agentic production pipelines** — OpenMontage demonstrates the shift from "AI as a single tool" to "AI as a full production crew." The 12-pipeline architecture covers every phase from scripting to delivery.

5. **Self-hosted studios surge** — Studios with IP protection requirements are choosing self-hosted (Open Generative AI, ComfyUI) over cloud APIs. MIT/Apache licensing is the key criteria.

6. **Agentic newsroom AI crosses tipping point** — Reuters Institute 2026 Digital News Report: 75% of news executives expect agentic tools to have a "large" or "very large" impact. Publishers forecast 40% decline in search referrals from AI answer engines (Google AI Overviews, ChatGPT). Newsrooms deploying GPT-Researcher + STORM patterns to automate research briefs and background reporting at scale, while keeping human editorial oversight for final copy.

7. **Kokoro TTS fills LATAM voice gap** — ElevenLabs per-character API costs are prohibitive for high-volume LATAM content. Kokoro-82M (Apache-2.0, CPU-viable, Spanish + Portuguese) is the open alternative for dubbing, podcast auto-voicing, and short-form narrator tracks. Combined with Whisper.cpp for ASR and Claude for translation, it closes the complete LATAM localization pipeline on-premise.
